...
type ClassDAO interface {
	...
	ListClassesBySubjectId(subjectId string, limit, offset int32) ([]*class.Class, error)
	ListClassesByTeacherAndSubject(teacherId, subjectId string, limit, offset int32) ([]*class.Class, error)
}
...
// ListClassesBySubjectId 根据科目ID查询班级列表
func (d *classDAOImpl) ListClassesBySubjectId(subjectId string, limit, offset int32) ([]*class.Class, error) {
	db := DB
	var classes []*class.Class
	err := db.Where("subject_id = ?", subjectId).Limit(int(limit)).Offset(int(offset)).Find(&classes).Error
	return classes, err
}

// ListClassesByTeacherAndSubject 根据教师ID和科目ID查询班级列表
func (d *classDAOImpl) ListClassesByTeacherAndSubject(teacherId, subjectId string, limit, offset int32) ([]*class.Class, error) {
	db := DB
	var classes []*class.Class
	err := db.Where("teacher_id = ? AND subject_id = ?", teacherId, subjectId).Limit(int(limit)).Offset(int(offset)).Find(&classes).Error
	return classes, err
}
